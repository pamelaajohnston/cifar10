...
def tower_loss(scope):
  ...
  images, labels = qpNet.distorted_inputs()
...
def train():
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    num_batches_per_epoch = (qpNet.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN / FLAGS.batch_size)
    decay_steps = int(num_batches_per_epoch * qpNet.NUM_EPOCHS_PER_DECAY)
    # decay_steps = 1500

    # Decay the learning rate exponentially based on the number of steps.
    lr = tf.train.exponential_decay(qpNet.INITIAL_LEARNING_RATE,
                                    global_step,
                                    decay_steps,
                                    qpNet.LEARNING_RATE_DECAY_FACTOR,
                                    staircase=True)
